...
gcloud run deploy $SERVICE_NAME_UI --region=$REGION --source ./UI --min-instances=1 --timeout=60m \
...
gcloud iap web enable --resource-type=backend-services --service=$SERVICE_NAME_UI-backend
...
# Escopo Global
gcloud compute backend-services update $SERVICE_NAME_UI-backend --global --iap=enabled
# OU Escopo Regional
gcloud compute backend-services update $SERVICE_NAME_UI-backend --region $REGION --iap=enabled
